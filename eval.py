...
if __name__ == "__main__":
  ...
  for d, data_file in enumerate(tqdm(files[:SAMPLE_ROLLOUTS], total=SAMPLE_ROLLOUTS)):
    test_cost, test_target_lataccel, test_current_lataccel = run_rollout(data_file, args.test_controller, args.model_path, debug=DEBUG)
    baseline_cost, baseline_target_lataccel, baseline_current_lataccel = run_rollout(data_file, args.baseline_controller, args.model_path, debug=DEBUG)
  ...
  for controller_cat, controller_type in [('baseline', args.baseline_controller), ('test', args.test_controller)]:
    ...
    rollout_partial = partial(run_rollout, controller_type=controller_type, model_path=args.model_path, debug=DEBUG)

...
class Controller(BaseController):
    ...
    def __init__(self):
        # Load the NEW model (ensure the filename matches your newly trained model)
        if Controller._shared_model is None:
            Controller._shared_model = PPO.load("models/new_ppo", device='cpu') 
    ...
    def update(self, target_lataccel, current_lataccel, state, future_plan):
        ...
        obs = np.array([
            ...
            self.pid.error_integral / 10.0,  # The AI sees if PID is winding up
            pid_steer / 2.0,                 # The AI sees what PID wants to do
